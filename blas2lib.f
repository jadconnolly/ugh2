...
c                                 load feasibility tolerances.
      w(lfeatl:lfeatl+nplin-1) = tolfea
      ...
      w(lwtinf:lwtinf+nplin-1) = w(lfeatl:lfeatl+nplin-1)/feamin
...
c                                the point as already computed.
      outrpc = .false.
...
c                                 end of cmmul1
      ...
      subroutine nnpiqp (feasqp,unitq,nqperr,minits,n,nclin,
     *                  ldaqp,ldr,linact,nlnact,nactiv,nfree,nz,
     *                  numinf,istate,kactiv,kx,dxnorm,gdx,qpcurv,aqp,
     *                  adx,ax,bl,bu,clamda,dx,qpbl,qpbu,qptol,r,x,
     *                  wtinf,w)
c----------------------------------------------------------------------
...
c     violations.
      ...
      if (wscale.gt.0d0) then
         wscale = 1d0/wscale
         call dscal (nplin,(wscale),wtinf,1)
      end if

      call scond (nplin,wtinf,1,wtmax,wtmin)
      wtmin = epspt9*wtmax
...
c     constraints in the working set.
      ...
      call npsetx(unitq,ncqp,nactiv,nfree,nz,n,nplin,ldzy,ldaqp,
...
c                                 end of npiqp
      ...
      subroutine wnpiqp (feasqp,unitq,nqperr,minits,n,nclin,
     *                  ldaqp,ldr,linact,nlnact,nactiv,nfree,nz,
     *                  numinf,istate,kactiv,kx,dxnorm,gdx,qpcurv,aqp,
     *                  adx,ax,bl,bu,clamda,
...
c                                 end of cmchzr
      ...
      subroutine xcmprt (nfree,n,nclin,nctotl,nactiv,kactiv,kx,clamda,
     *                  rlamda)
c----------------------------------------------------------------------
...
c                                 end of npfeas
      ...
      subroutine xnpsrch (needfd,inform,n,nfun,ngrad,
     *                  objfun,alfa,alfmax,alfsml,
     *                  dxnorm,epsrf,eta,gdx,grdalf,glf,objf,
     *                  objalf,xnorm,
     *                  dx,grad,gradu,x1,x,bl,bu)
c----------------------------------------------------------------------
...
c                                 end of npsrch
      ...
      subroutine xnpupdt (lsumry,n,ldr,alfa,glf1,glf2,qpcurv,
...
c----------------------------------------------------------------------
     ...
     *                  c(*), clamda(nctotl), featol(nctotl), grad(n),
...
c                                 and multipliers.
     ...
     *            bl,bu,clamda
     *            ,w(ldx),w(lbl),w(lbu),w(lqptol),r,x,w(lwtinf),w)
c            call npiqp (feasqp,unitq,nqperr,mnr,n,nclin,
c     *            ldaqp,ldr,linact,nlnact,nactiv,nfree,nz,numinf,istate,
c     *            kactiv,kx,dxnorm,gdx,qpcurv,aqp,w(ladx),ax,
c     *            bl,bu,clamda,w(ldx),w(lbl),w(lbu),w(lqptol),r,
c     *            x,w(lwtinf),w)
c
            minits = minits + mnr
            mnrsum = mnrsum + mnr
...
c        defined limit alflim on the change in x.
     ...
     *                  dxnorm,w(lanorm),w(ladx),ax,bl,bu,
     *                  w(ldx),x)
            alfmax = alfa
...
c                                 end of lscore
      ...
      subroutine xcmprnt (nfree,n,nclin,nctotl,
